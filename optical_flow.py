...
def start_tracking():
    ...
    track_params = dict(winSize=(11, 11), maxLevel=2,
    ...
    while True:
        ...
        if len(track_paths) > 0:
            ...
            feature_points_0 = np.float32([tp[-1] for tp in track_paths]
                                          ).reshape(-1, 1, 2)
            ...
            for tp, (x, y), good_points_flag in zip(track_paths,
            ...
            track_paths = new_tracking_paths

            # Draw lines
            cv2.polylines(output_img, [np.int32(tp) for tp in track_paths], False, (0, 150, 0))

        # Go into this 'if' condition after skipping the
        # right number of frames
        if not frame_index % num_jump:
            # Create a mask and draw the circles
            mask = np.zeros_like(frame_gray)
            mask[:] = 255
            for x, y in [np.int32(tp[-1]) for tp in track_paths]:
